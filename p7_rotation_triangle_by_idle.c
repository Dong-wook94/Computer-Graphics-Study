...
void myidle(void) {
	...
	for (i = 0; i < 3; i++) { // for each vertex,
		vertices[4 * i + 0] = ref[4 * i + 0] * cos(theta) - ref[4 * i + 1] * sin(theta);
		vertices[4 * i + 1] = ref[4 * i + 0] * sin(theta) + ref[4 * i + 1] * cos(theta);
		vertices[4 * i + 2] = ref[4 * i + 2];
		vertices[4 * i + 3] = ref[4 * i + 3];
	}
	// redisplay
	glutPostRedisplay();
}
